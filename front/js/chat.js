...
class AudioManager {
    ...
    async startRecording() {
        try {
            ...
            // 미디어 레코더 옵션 설정
            const options = {
                audioBitsPerSecond: 128000  // 128kbps 오디오 품질
            };
            ...
            this.mediaRecorder.ondataavailable = (event) => {
                if (event.data.size > 0) {  // 데이터 크기가 0보다 큰 경우에만 처리
                    ...
                    console.log(`오디오 데이터 청크 수신: ${event.data.size} 바이트, 형식: ${event.data.type}`);
                } else {
                    console.warn('빈 오디오 데이터 청크가 수신되었습니다.');
                }
            };
            
            // 녹음 시작 시점에도 데이터를 요청 (일부 브라우저에서 필요함)
            setTimeout(() => {
                if (this.mediaRecorder && this.mediaRecorder.state === 'recording') {
                    this.mediaRecorder.requestData();
                }
            }, 1000); // 녹음 시작 1초 후 데이터 요청
            ...
            this.mediaRecorder.start(10);  // 레코딩 시작, 10ms마다 데이터 전송 (더 작은 청크 단위로 수집)
